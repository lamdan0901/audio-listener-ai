// Animation state - these are now defined in main.js as window properties
// This file just provides functions to manipulate them
...
function processNextAnimation() {
  console.log(
    "processNextAnimation called, queue length:",
    window.animationQueue.length
  );

  if (window.animationQueue.length === 0) {
    window.animationInProgress = false;
    console.log("Animation queue empty, exiting processNextAnimation");
    return;
  }

  window.animationInProgress = true;
  console.log("Setting animationInProgress to true");
  const nextContent = window.animationQueue.shift();
  console.log(
    "Shifted next content from queue, remaining items:",
    window.animationQueue.length
  );
  ...
  if (!contentElement) {
    console.error("streamingContent element not found in processNextAnimation");
    window.animationInProgress = false;
    return;
  }

  console.log("Found streamingContent element");

  // Handle initial content
  if (!contentElement.innerHTML.trim()) {
    console.log("Initial content case - empty streamingContent");
    ...
    console.log("Appended initial content to streamingContent");
    ...
    if (window.animationQueue.length > 0) {
      console.log("More items in queue, scheduling next animation");
      setTimeout(processNextAnimation, 50);
    } else {
      window.animationInProgress = false;
      console.log(
        "No more items in queue, setting animationInProgress to false"
      );
    }
    return;
  }

  console.log("Subsequent content case - updating existing content");
