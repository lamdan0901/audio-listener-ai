...
const renderConfig = {
  ...
  pngScale: 3,           // Scale factor for PNG (higher = better resolution)
  pngQuality: 100,       // Quality for PNG (1-100)
...
files.forEach(file => {
  ...
  if (mermaidMatch && mermaidMatch[1]) {
    ...
    // Render in each output format
    outputFormats.forEach(format => {
      const outputFilePath = path.join(outputDir, `${baseName}${format.ext}`);
      
      // Build command with appropriate options
      let command = `mmdc -i "${tempMermaidFile}" -o "${outputFilePath}" -c "${configFile}"`;
      
      // Add format-specific options
      if (format.type === 'png') {
        command += ` -s ${renderConfig.pngScale} -q ${renderConfig.pngQuality}`;
      }
      
      console.log(`Rendering ${baseName} to ${format.type.toUpperCase()}...`);
      
      // Execute the command
      exec(command, (error, stdout, stderr) => {
        if (error) {
          console.error(`Error rendering ${file} to ${format.type}: ${error.message}`);
          return;
        }
        
        if (stderr && !stderr.includes('Generating')) {
          console.error(`Warning while rendering ${file} to ${format.type}: ${stderr}`);
        }
        
        console.log(`Successfully rendered ${file} to ${format.type.toUpperCase()}: ${outputFilePath}`);
      });
    });
...
console.log('Rendering process started. Check the images directory for output files.');
console.log('Note: SVG files provide the best quality for viewing and embedding in documents.');
